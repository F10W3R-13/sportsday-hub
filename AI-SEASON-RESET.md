...
AI는 다음 구조를 **매 학기 동일하게 유지된다고 가정**하고 파싱한다 (26-2 사용자 확인):

- 배치표 시트2의 열 체계 (시간블록→역할→담당자→멤버) — 내용만 교체, 구조 불변
- 게임 유형 틀 (토너먼트형 + 메인게임 + 미니부스) — 게임 내용은 새로, 분류는 유지
- 5팀 체제 — 팀 id 슬러그 고정

**행사 장소 주의**: 1학기는 **명륜** 개최 (율전 조가 버스), 2학기는 **율전** 개초 (명륜 조가 버스).
GATHER 구조가 학기에 따라 뒤집히므로, 최종기획안의 장소를 먼저 확인하고
`my-role data.ts`의 GATHER 방향을 맞춰야 한다.
